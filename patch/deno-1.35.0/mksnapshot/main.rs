// deno-1.35.0/mksnapshot
// code from deno-1.35.0/build.rs
...
fn main() {
    println!("deno-mksnapshot: deno-1.35.0");
    ...
    println!("  {}", compiler_snapshot_path.display());
    ts::create_compiler_snapshot(compiler_snapshot_path, &c);

    let cli_snapshot_path = o.join("CLI_SNAPSHOT.bin");
    println!("  {}", cli_snapshot_path.display());
    let output = create_cli_snapshot(cli_snapshot_path);
    // for path in output.files_loaded_during_snapshot {
    //     println!("cargo:rerun-if-changed={}", path.display())
    // }

    println!("deno-mksnapshot: done");
}
